...
<div class="container">
    ...
    <ul class="nav nav_main">
        <?php foreach ($categogies as $categogy) { ?>
            <li><a style="text-transform: capitalize; padding:10px 20px;" href="<?= $SHOP_URL ?>?page=1&category=<?= $categogy['id_category'] ?>"><?= $categogy['name'] ?></a>
            </li>
        <?php
        }
        ?>
        ...
        <nav aria-label="Page navigation example" style="margin-top: 20px;">
            <ul class="pagination justify-content-center">
                <li class='page-item <?php if ($page <= 1) echo 'disabled'; ?>'>
                    <a class="page-link link_nav_shop" href="<?= $SHOP_URL ?>?page=<?= $page - 1 ?>">Previous</a>
                </li>
                <li class="page-item"><a class="page-link link_nav_shop <?php if ($page == 1) echo 'link_nav_shop_active' ?>" href="<?= $SHOP_URL ?>?page=1">1</a></li>
                <?php
                for ($i = 2; $i <= $totalpage - 1; $i++) { ?>
                    <li class="page-item">
                        <a class="page-link link_nav_shop <?php if ($page == $i) echo 'link_nav_shop_active' ?>" href="<?= $SHOP_URL ?>?page=<?= $i ?>"><?= $i ?></a>
                    </li>
                <?php
                }
                ?>
                <li class="page-item"><a class="page-link link_nav_shop <?php if ($page == $totalpage) echo 'link_nav_shop_active'; ?>" href="<?= $SHOP_URL ?>?page=<?= $totalpage ?>"><?= $totalpage ?></a></li>
                <li class='page-item <?php if ($page > $totalpage - 1) echo 'disabled' ?>'>
                    <a class="page-link link_nav_shop" href="<?= $SHOP_URL ?>?page=<?= $page + 1 ?>">Next</a>
                </li>
            </ul>
        </nav>
    </div>
